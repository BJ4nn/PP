...
import { getCompanyHoursCsv } from "@/server/services/exports";
...
export async function GET(request: Request) {
  ...
  const csv = await getCompanyHoursCsv(session.user.id, request.url);
  const url = new URL(request.url);
  const jobId = url.searchParams.get("jobId");
  const rangeDays = parseRangeDays(url.searchParams.get("range"));
  const filename = jobId
    ? `worked-hours_job-${safeFilenamePart(jobId)}.csv`
    : `worked-hours_last-${rangeDays}d.csv`;
