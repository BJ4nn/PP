...
export function CompanyContractSignForm({
  contractId,
  ...
  const sign = () => {
    startTransition(async () => {
      setError(null);
      setSaved(false);
      const res = await fetch(`/api/company/contracts/${contractId}/sign`, {
        method: "POST",
  ...
  return (
    <section className="space-y-4 rounded-3xl border border-border bg-card p-6 shadow-sm">
      <div>
        <h2 className="text-lg font-semibold text-foreground">Podpis firmy (Beta)</h2>
        <p className="text-sm text-muted-foreground">
          Zadajte meno podpisujúcej osoby a potvrďte podpis.
        </p>
      </div>
      <div className="space-y-1">
        <Label htmlFor="companySignatureName">Meno a priezvisko</Label>
        <Input
